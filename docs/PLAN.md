...
| **PST archive → names and descriptions** | ✅ **DONE.** `L10N/<lang>/` overrides in the **client** pak | **Corrected then delivered 2026-08-05.** This row previously named `Game.locres`, wrong twice over: all 17 of those archives are 37-byte placeholders with zero entries, and the server pak's `*Text` tables are not "opaque" — they decode, and they are **Japanese**. `scripts/l10n.py` reads the real source (235,696 of 235,696 rows); `scripts/gametext.py` joins it; `build-gamedata.py` overlays it. `activeSkills` disagrees with the archive **0 times of 326**, and technology's 410 apparent disagreements collapsed to **3** once `<itemName id=\|X\|/>` was resolved — all three the archive shipping a raw id. Gained 1,805 item and 303 Paldeck descriptions, 140 region names and 33 dungeon names. |
