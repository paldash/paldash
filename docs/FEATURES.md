...
| Do level and stars change a speed? | ⚠️ | **Unsettled, and this row used to claim they do not.** The files carry no build term beside `RideSprintSpeed` — that is a fact about the files, not about the game, and the condenser's effect on work suitability was applied at load and invisible the same way. Passives demonstrably do move it. Only an in-game timing run settles the rest |
