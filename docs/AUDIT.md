...
### Phase 10 — Features needing one new extraction — **DONE**

Each was one bundle away, and the bundle was small. All five are complete.
...
| #71 | Boss encounter planner | **done** — `bossplanner.py` + `/api/world/encounters` + a card on the Progression tab. 90 field / 11 raid / 8 tower, kept in separate groups because the kinds are not comparable |
...
| #46 | Re-check what the client-pak wall ruled out | **done.** Most of its checklist was closed by #48/#49/#35/#36; the two left are now settled. **The element chart is absent by ENUMERATION** — exactly 14 packages name all nine elements and every one is accounted for, including the two CDOs the task asked about (`BP_PalWazaDatabase`, `BP_PalItemIDManager`), which decode cleanly and hold ailment/idle-skill/egg maps rather than any element-to-element relation. **`_Common` twins are identical** for every non-text table read here, so `DT_ItemShopCreateData` vs `_Common` does not matter. Turned up a reader limitation worth more than either: **a terminating walk proves alignment, not values** — `MeshRelativeLocation` decodes to junk on 97 rows while the walk stays perfect, blast radius zero |
...
| #86 | `MapObjectSpawnerInStageSaveData` | **done — hypothesis confirmed, feature blocked, and the blocker is named.** It is the respawn clock for every world gatherable, verified by a contingency table with **zero violations across three worlds and ~99,800 slots**: a spawner whose object is standing never has a timer, and one with a timer never has an object. Two traps caught — the outer map is keyed by STAGE and `[0]` drops the dungeon instances (`base_camp_level` again), and `DateTime.MaxValue` means *never* but reads as 87,637,883 hours. **No map layer**: the spawner key is a level-object id resolving against nothing, and `worldobjects.json.gz` has positions but no GUIDs. `scripts/decode-spawner-state.py` |
...
| #89 | Achievement progress | **done** — `achievements.py`, on the Progression tab. Steam's remain a refusal (external API, per-account). The game's own milestone NPC is 26 rows in three categories, and **the expected hard part did not exist**: `NPCAchivementRewardFlag` names the row outright, 26 of 26 claimed keys resolve. `BossDefeat` ships `counter: null` — no counter is establishable, so it renders claimed tiers and no progress. Found a shipped bug on the way: **`TribeCaptureCount` is a plain int and the parser read it as a map**, so `speciesCaptured` was 0 for every player against real values of 210/149/128/109/8 |

**All seven closed, and four closed as negatives** — which is the
